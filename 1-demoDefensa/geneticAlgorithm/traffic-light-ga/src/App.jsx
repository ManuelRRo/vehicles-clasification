...
const TrafficLightGA = () => {
  ...
  const [zones, setZones] = useState([
    { id: 1, name: 'Norte', vehicles: 45, minTime: 20, maxTime: 90 },
    { id: 2, name: 'Sur', vehicles: 38, minTime: 20, maxTime: 90 },
    { id: 3, name: 'Este', vehicles: 52, minTime: 20, maxTime: 90 },
    { id: 4, name: 'Oeste', vehicles: 30, minTime: 20, maxTime: 90 }
  ]);
    ...
    <div className="w-full max-w-6xl mx-auto p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">
          Optimización de Semáforos usando Algoritmos Genéticos
        </h1>
        <p className="text-gray-600">
          Optimizando la duración de luz verde para cada zona basado en el conteo de vehículos
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                {Object.entries(params).map(([key, value]) => (
                  <div key={key}>
                    <label className="block text-sm font-medium text-gray-700 mb-1 capitalize">
                      {key === 'populationSize' ? 'Tamaño Población' :
                       key === 'mutationRate' ? 'Tasa Mutación' :
                       key === 'crossoverRate' ? 'Tasa Cruce' :
                       key === 'elitismCount' ? 'Élites' :
                       'Máx. Generaciones'}
                    </label>
                    <input
                      type="number"
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            <div className="text-sm text-gray-600 mb-1">Generación</div>
            <div className="text-3xl font-bold text-blue-600">
              {generation} / {params.maxGenerations}
            </div>
            <div className="mt-3 text-sm text-gray-600">Mejor Fitness</div>
        ...
        {/* Traffic Zones Input */}
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-semibold mb-4 text-gray-800">Zonas de Tráfico</h2>
            ...
            {zones.map((zone, idx) => (
              <div key={zone.id} className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-600">Zona {zone.id}</span>
                </div>
        ...
        {/* Best Solution */}
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-semibold mb-4 text-gray-800">Tiempos Optimizados</h2>
          ...
          {bestSolution && (
            <div className="space-y-4">
              {bestSolution.chromosome.map((gene) => {
                ...
                return (
                  <div key={gene.zoneId} className="p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-gray-800">{zone.name}</span>
                      <span className="text-sm text-gray-600">{zone.vehicles} vehículos</span>
                    </div>
                    ...
                    <div className="mt-2 text-xs text-gray-600">
                      {percentage.toFixed(1)}% del ciclo total
                    </div>
                  </div>
                );
              })}
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                <div className="text-sm text-gray-600">Tiempo Total del Ciclo</div>
                <div className="text-2xl font-bold text-blue-600">
                  {totalCycleTime} segundos
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  (incluye 3s de transición por zona)
                </div>
              </div>
            </div>
          )}
      ...
      {/* Fitness Chart */}
      <div className="bg-white rounded-xl shadow-lg p-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-800">Evolución del Fitness</h2>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
          <div>
            <h3 className="font-semibold text-gray-800 mb-2">1. Codificación del Cromosoma</h3>
            <p>Cada solución es un conjunto de duraciones de luz verde para todas las zonas.</p>
          </div>
          
          <div>
            <h3 className="font-semibold text-gray-800 mb-2">2. Función de Fitness</h3>
            <p>Minimiza el tiempo total de espera de vehículos balanceando throughput y eficiencia del ciclo.</p>
          </div>
          
          <div>
            <h3 className="font-semibold text-gray-800 mb-2">3. Selección</h3>
            <p>Selección por torneo elige mejores soluciones con mayor probabilidad.</p>
          </div>
          
          <div>
            <h3 className="font-semibold text-gray-800 mb-2">4. Evolución</h3>
            <p>El cruce combina soluciones, la mutación explora variaciones, el elitismo preserva las mejores.</p>
          </div>
        </div>
      </div>
    </div>
